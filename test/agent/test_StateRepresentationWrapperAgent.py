...
class TestStateRepresentationWrapperAgent(TestCase):
    ...
    def test_get_abstract_agent(self):
        ...
        agent = rl.agent.StateRepresentationWrapperAgent(agent_latent, lambda s: np.matmul(s, rep_mat))
    ...
    def test_reset(self):
        ...
        import numpy as np
        rep_mat = self._get_representation_mat()
        agent_latent = rl.agent.QLearning(num_states=3, num_actions=2, learning_rate=0.1, gamma=0.9, init_Q=0.1)
        agent = rl.agent.StateRepresentationWrapperAgent(agent_latent, lambda s: np.matmul(s, rep_mat))
        res = agent.update_transition(rl.one_hot(0, 9), 0, 0., rl.one_hot(1, 9), term=False, info={})
    ...
    def test_q_values(self):
        ...
        agent_latent = rl.agent.VIAgent(t_mat, r_vec, gamma, eps=1e-5)
        agent = rl.agent.StateRepresentationWrapperAgent(agent_latent, lambda s: np.matmul(s, rep_mat))
        q_vec = np.stack([agent.q_values(rl.one_hot(i, 9)) for i in range(9)]).transpose()
        q_corr_infl = np.stack([np.matmul(rep_mat, q_corr[a]) for a in range(2)])
        self.assertLessEqual(np.max(np.abs(q_vec - q_corr_infl)), 1e-4)
